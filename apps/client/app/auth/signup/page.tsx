    ...
    <div className="min-h-screen bg-background grid-bg flex items-center justify-center px-6">
      <div className="w-full max-w-sm animate-fade-up">
        <div className="text-center mb-8">
          ...
          {step === "form" ? (
            <>
              ...
              <p className="text-sm text-dim">Start building real-time conversations today</p>
            </>
          ) : (
            <>
          ...
          {step === "form" ? (
            <>
              ...
              <div className="flex items-center gap-4 my-5">
                ...
                <span className="text-[11px] text-dim uppercase tracking-wider">or</span>
                <div className="flex-1 h-px bg-border" />
              </div>

              <form onSubmit={handleSignup} className="space-y-3">
                <div>
                  <label className="block text-[11px] text-dim uppercase tracking-wider font-medium mb-1.5">Name</label>
                  <input
                    type="text"
                    value={name}
                ...
                <div>
                  <label className="block text-[11px] text-dim uppercase tracking-wider font-medium mb-1.5">Email</label>
                  <input
                    type="email"
                    value={email}
                ...
                <div>
                  <label className="block text-[11px] text-dim uppercase tracking-wider font-medium mb-1.5">Password</label>
                  <input
                    type="password"
                    value={password}
                ...
                {error && (
                  <p className="text-[13px] text-rose-400 bg-rose-400/10 border border-rose-400/20 rounded-xl px-3.5 py-2.5">{error}</p>
                )}
            ...
            <form onSubmit={handleVerifyOtp} className="space-y-3">
              <div>
                <label className="block text-[11px] text-dim uppercase tracking-wider font-medium mb-1.5">Verification code</label>
                <input
                  type="text"
                  value={otp}
        ...
        <p className="text-center text-[13px] text-dim mt-6">
          ...
          <Link href="/auth/signin" className="text-emerald hover:text-emerald/80 transition-colors">Sign in</Link>
        </p>
      </div>
    </div>
  );
